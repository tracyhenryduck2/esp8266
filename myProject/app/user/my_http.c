...
void ICACHE_FLASH_ATTR HttpReadFile(char *URL,char *method,char *postdata){
	...
	//用来解析域名得到IP，如果是本地ip就直接执行下面代码
//	espconn_gethostbyname(&user_tcp2_conn,host, &addr,
//	user_esp_dns_found);
	const char remote_ip[4]={192,168,3,140};//hekr服务器
	wifi_get_ip_info(STATION_IF,&info);
...
void ICACHE_FLASH_ATTR HttpReadBinUrl(char *URL,char *method,char *postdata){
	struct ip_addr addr;
	memset(binbuffer,0,1024);
	http_parse_request_url(URL,host,filename,&port);
	if(strcmp(method,"GET")==0){
		os_sprintf(binbuffer,GET,filename,host);
	}else{
		os_sprintf(binbuffer,POST,filename,strlen(postdata),host,postdata);
	}
